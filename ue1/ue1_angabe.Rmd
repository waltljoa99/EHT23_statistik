...
confint(model, level=0.95)
```
Laut der p-Werte (p<0.01) und zugehörigen Konfidenzintervalle liefert das lineare Regressionsmodell
hochsignfikante Ergebnisse für die Konstante und den Koeffizienten in
...
D.h. das linerare Modell ist kein gut geeignetes für den Datensatz.
...
lineare Modell, ein logarithmesches, ein quadratisches, kubisches und
...
Muster erkennen lassen. Das quadratische Modell ist daher hinreichend gut.
...
Nicht erklärbar ist der positive Wert von bpmeds1
(vorhandene Einnahme von Blutdrucksenkern). Dieses hochsignifikatne Ergebnis
besagt, dass Blutdrucksenker den systolischen Blutdruck erhöhen!
...
p13 <- scatter_plot(df, "heartrte", title = "Streudiagramm Herzrate")
...
```
Der systolische Blutdruck der Person mit dem gegebenen Profil ist 138.4 mmHg
