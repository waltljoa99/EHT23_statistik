...
conf_intervals <- confint(max_model)
...
model <- lm(sysbp ~ sex + age + educ + cigpday + bpmeds + totchol + diabp + bmi + diabetes + heartrte + glucose, data = data)
...
model_data <- data[complete.cases(data), ]  # remove rows with missing values
df <- data.frame(resid = resid(model), fitted = fitted(model),
                 sysbp = model_data$sysbp, bmi = model_data$bmi)
...
# 4. Scatter Plot
p4 <- ggplot(df, aes(x=sysbp, y=sex)) +
  geom_jitter(width=0.0) +
  ggtitle("Streudiagramm Gewicht")

# 5. Scatter Plot
p5 <- ggplot(df, aes(x=sysbp, y=age)) +
  geom_jitter(width=0.0) +
  ggtitle("Streudiagramm Gewicht")

# 6. Scatter Plot
p6 <- ggplot(df, aes(x=sysbp, y=bmi)) +
  geom_jitter(width=0.0) +
  ggtitle("Streudiagramm Gewicht")

# 7. Scatter Plot
p7 <- ggplot(df, aes(x=sysbp, y=bpmeds)) +
  geom_jitter(width=0.0) +
  ggtitle("Streudiagramm Gewicht")

# 8. Scatter Plot
p8 <- ggplot(df, aes(x=sysbp, y=totchol)) +
  geom_jitter(width=0.0) +
  ggtitle("Streudiagramm Gewicht")

# sex + age + educ + cigpday + bpmeds + totchol + diabp + bmi + diabetes + heartrte + glucose
