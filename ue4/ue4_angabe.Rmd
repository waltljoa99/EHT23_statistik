...
# Glucose
means <- aggregate(glucose~group,diabetes,mean)
medians <- aggregate(glucose~group,diabetes,median)
sd <- aggregate(glucose~group,diabetes,FUN = "sd")
  
  # Insulin
insulin_mean <- aggregate(insulin~group,diabetes,mean)
# fpg
fpg_mean <- aggregate(fpg~group,diabetes,mean)
# sspg
sspg_mean <- aggregate(sspg~group,diabetes,mean)
# rw
rw_mean <- aggregate(rw~group,diabetes,mean)
...
# Normalverteilung der Prädiktoren (strenge Überprüfung)
for (i in 1:5) {
