...
>>>>>>> d4d2fae752e771b9232904acfed0be90e0d715e5
...
aggregate(glucose~group,diabetes,mean)
aggregate(glucose~group,diabetes,median)
aggregate(glucose~group,diabetes,sd)

aggregate(insulin~group,diabetes,mean)
aggregate(fpg~group,diabetes,mean)
aggregate(sspg~group,diabetes,mean)
aggregate(rw~group,diabetes,mean)

# Histogramme ansehen
ggplot(diabetes,aes(glucose,col=group,fill=group)) +geom_histogram(alpha=0.5,position="identity")
ggplot(diabetes,aes(insulin,col=group,fill=group)) +geom_histogram(alpha=0.5,position="identity")
ggplot(diabetes,aes(fpg,col=group,fill=group)) +geom_histogram(alpha=0.5,position="identity")
ggplot(diabetes,aes(sspg,col=group,fill=group)) +geom_histogram(alpha=0.5,position="identity")
ggplot(diabetes,aes(rw,col=group,fill=group)) +geom_histogram(alpha=0.5,position="identity")

```
Anhand der Histogramme lässt sich kein Prädiktor identifizieren, der eine klare Trennung zwischen den Klassen erlaubt.
=======
# Laden der Daten
diabetes_RM <- read.csv("diabetes_RM.csv")
# Explorative Analyse der Daten

# Histogramme
create_histograms(diabetes_RM)
```
>>>>>>> d4d2fae752e771b9232904acfed0be90e0d715e5
...
for (i in 2:6) {
  print(shapiro.test(diabetes[,i]))
}
=======
# Check for normality
normality_tests <- apply(diabetes_RM[, sapply(diabetes_RM, is.numeric)], 2, shapiro.test)
print(normality_tests)

# Check for homogeneity of variances
diabetes_RM$group <- as.factor(diabetes_RM$group) # Levene test requires a factor 
numeric_columns <- diabetes_RM[, sapply(diabetes_RM, is.numeric), drop = FALSE]
levene_tests <- lapply(numeric_columns, function(column) car::leveneTest(column, diabetes_RM$group))
print(levene_tests)

# Check for multivariate normality
mvn_test <- MVN::mvn(diabetes_RM[, sapply(diabetes_RM, is.numeric)], mvnTest = "hz")
print(mvn_test)

# Check for equality of covariance matrices
box_m_test <- biotools::boxM(diabetes_RM[, sapply(diabetes_RM, is.numeric)], diabetes_RM$group)
print(box_m_test)
>>>>>>> d4d2fae752e771b9232904acfed0be90e0d715e5
```
