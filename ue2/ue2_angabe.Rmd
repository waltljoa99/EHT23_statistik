...
```{r}
# The `here` package uses a heuristic to find your project's root directory
# based on certain criteria (like the presence of a `.Rproj` file,
# a `.git` directory, or a `.here` file).
# Therefore, the directory returned by here() is not necessary equal
# to the current working directory returned by getwd().

# Parameter 'relative_path' should be the relative path
# from the directory returned by here() to the parent directory
# of the files you are working with.
# This directory will be set as the working directory.
set_working_directory <- function(relative_path) {
  library(here)
  path_to_working_directory <- file.path(here(), relative_path)
  if(getwd() != path_to_working_directory) {
    setwd(path_to_working_directory)
  }
  return(getwd())
}

# The working direcotry is only changed if the code is not run in RStudio
isRStudio <- Sys.getenv("RSTUDIO") == "1"
if (!isRStudio) {
  # Change relative path accordingly to your working directory
  print(set_working_directory("ue2"))
}
```
